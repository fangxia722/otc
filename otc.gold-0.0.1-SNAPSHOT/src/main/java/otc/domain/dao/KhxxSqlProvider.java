...
public class KhxxSqlProvider {
	...
	public String querySelective(Khxx record) {
		BEGIN();
		SELECT("ID, KHH, KHXM, KHQC, PYDM, YYB, KHLX, ZJLB, ZJBH, ZJDZ, ZJYXQ, KHZT, WTFS, FWXM");
		SELECT("KHJL, TZGW, KHKH, KHRQ, XHRQ, KHSX, TZZFL, FXCSNL, TZPZ, TZQX, PCRQ, QDWD, YWFW");
		SELECT("KHJB, KHQZ, SJ, DH, EMAIL, CZ, QQ, MSN, DZ, YZBM, PROVINCE, CITY, SEC, SEX");
		SELECT("CSRQ, HYZK, ZYDM, XL, DWMC, HYLB, DWXZ, XZZW, DWDZ, DWDH, DWYB, TBTS, GJ, MZ");
		FROM("TFP_KHXX");
		if (record.getKhh() != null) {
			WHERE("KHH = #{khh,jdbcType=VARCHAR}");
		}
		if (record.getSj() != null) {
			WHERE("SJ = #{sj,jdbcType=VARCHAR}");
		}
		return SQL();
	}
}