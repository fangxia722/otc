package otc.domain.model;

import java.math.BigDecimal;

public class Khtzzh {
    private Long id;

    private Long khh;

    private Long yyb;

    private Long fxjg;

    private String tzzh;

    private String zhmc;

    private String zhmm;

    private String jyzh;

    private String jszh;

    private Long jslx;

    private String jsjg;

    private Long fhfs;

    private Long zhzt;

    private BigDecimal zhsx;

    private BigDecimal jyqx;

    private Long zdsx;

    private String hybh;

    private String pbu;

    private Long jsbz;

    private Long zhfxdj;

    private Long fxjbFxf;

    private Integer pcrq;

    private Integer khrq;

    private Integer xhrq;

    private String sj;

    private String dh;

    private String cz;

    private String dz;

    private String yzbm;

    private String khhDc;

    private Long ydsqybz;

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public Long getKhh() {
        return khh;
    }

    public void setKhh(Long khh) {
        this.khh = khh;
    }

    public Long getYyb() {
        return yyb;
    }

    public void setYyb(Long yyb) {
        this.yyb = yyb;
    }

    public Long getFxjg() {
        return fxjg;
    }

    public void setFxjg(Long fxjg) {
        this.fxjg = fxjg;
    }

    public String getTzzh() {
        return tzzh;
    }

    public void setTzzh(String tzzh) {
        this.tzzh = tzzh == null ? null : tzzh.trim();
    }

    public String getZhmc() {
        return zhmc;
    }

    public void setZhmc(String zhmc) {
        this.zhmc = zhmc == null ? null : zhmc.trim();
    }

    public String getZhmm() {
        return zhmm;
    }

    public void setZhmm(String zhmm) {
        this.zhmm = zhmm == null ? null : zhmm.trim();
    }

    public String getJyzh() {
        return jyzh;
    }

    public void setJyzh(String jyzh) {
        this.jyzh = jyzh == null ? null : jyzh.trim();
    }

    public String getJszh() {
        return jszh;
    }

    public void setJszh(String jszh) {
        this.jszh = jszh == null ? null : jszh.trim();
    }

    public Long getJslx() {
        return jslx;
    }

    public void setJslx(Long jslx) {
        this.jslx = jslx;
    }

    public String getJsjg() {
        return jsjg;
    }

    public void setJsjg(String jsjg) {
        this.jsjg = jsjg == null ? null : jsjg.trim();
    }

    public Long getFhfs() {
        return fhfs;
    }

    public void setFhfs(Long fhfs) {
        this.fhfs = fhfs;
    }

    public Long getZhzt() {
        return zhzt;
    }

    public void setZhzt(Long zhzt) {
        this.zhzt = zhzt;
    }

    public BigDecimal getZhsx() {
        return zhsx;
    }

    public void setZhsx(BigDecimal zhsx) {
        this.zhsx = zhsx;
    }

    public BigDecimal getJyqx() {
        return jyqx;
    }

    public void setJyqx(BigDecimal jyqx) {
        this.jyqx = jyqx;
    }

    public Long getZdsx() {
        return zdsx;
    }

    public void setZdsx(Long zdsx) {
        this.zdsx = zdsx;
    }

    public String getHybh() {
        return hybh;
    }

    public void setHybh(String hybh) {
        this.hybh = hybh == null ? null : hybh.trim();
    }

    public String getPbu() {
        return pbu;
    }

    public void setPbu(String pbu) {
        this.pbu = pbu == null ? null : pbu.trim();
    }

    public Long getJsbz() {
        return jsbz;
    }

    public void setJsbz(Long jsbz) {
        this.jsbz = jsbz;
    }

    public Long getZhfxdj() {
        return zhfxdj;
    }

    public void setZhfxdj(Long zhfxdj) {
        this.zhfxdj = zhfxdj;
    }

    public Long getFxjbFxf() {
        return fxjbFxf;
    }

    public void setFxjbFxf(Long fxjbFxf) {
        this.fxjbFxf = fxjbFxf;
    }

    public Integer getPcrq() {
        return pcrq;
    }

    public void setPcrq(Integer pcrq) {
        this.pcrq = pcrq;
    }

    public Integer getKhrq() {
        return khrq;
    }

    public void setKhrq(Integer khrq) {
        this.khrq = khrq;
    }

    public Integer getXhrq() {
        return xhrq;
    }

    public void setXhrq(Integer xhrq) {
        this.xhrq = xhrq;
    }

    public String getSj() {
        return sj;
    }

    public void setSj(String sj) {
        this.sj = sj == null ? null : sj.trim();
    }

    public String getDh() {
        return dh;
    }

    public void setDh(String dh) {
        this.dh = dh == null ? null : dh.trim();
    }

    public String getCz() {
        return cz;
    }

    public void setCz(String cz) {
        this.cz = cz == null ? null : cz.trim();
    }

    public String getDz() {
        return dz;
    }

    public void setDz(String dz) {
        this.dz = dz == null ? null : dz.trim();
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm == null ? null : yzbm.trim();
    }

    public String getKhhDc() {
        return khhDc;
    }

    public void setKhhDc(String khhDc) {
        this.khhDc = khhDc == null ? null : khhDc.trim();
    }

    public Long getYdsqybz() {
        return ydsqybz;
    }

    public void setYdsqybz(Long ydsqybz) {
        this.ydsqybz = ydsqybz;
    }
}